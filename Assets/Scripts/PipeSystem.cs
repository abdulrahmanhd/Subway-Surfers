...
public class PipeSystem : MonoBehaviour {
	...
	public Pipe SetupFirstPipe () {
		int cascadePipe = 0;
		bool cascadePresent = Random.Range(0, 100) > 0;
		if (cascadePresent) {
			cascadePipe = Random.Range(pipeCount/3, pipeCount/3*2);
		}

		for (int i = 0; i < pipes.Length; i++) {
			Pipe pipe = pipes [i];
			if (i == cascadePipe) {
				pipe.Generate (i > emptyPipeCount, true);
			} else {
	...
	public Pipe SetupNextPipe () {
		...
		pipes[pipes.Length - 1].GenerateWater();
